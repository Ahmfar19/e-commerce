...
const updateKlarnaOrderLines = async (klarnaOrder, deletedItems, updatedItems, newAmount) => {
    ...
    // console.error('klarnaOrder.id', klarnaOrder.id);
...
const updateKlarnaOrder = async (req, res, transaction) => {
    ...
    try {
        ...
        if (klarnaOrder.status === ORDER_STATUS.AUTHORIZED) {
            // Updatre the klarna not captured order
            if (refundAmount && newSubTotal) {
                ...
                const updatedKlarnaOrder = await updateKlarnaOrderLines(
                    klarnaOrder,
                    deletedItems,
                    updatedItems,
                    newSubTotal,
                ...
                if (updateResult.success) {
                    transaction?.commit();
                    return sendResponse(res, 200, 'Klarna order has been updated', 'Success', null, updatedKlarnaOrder);
                } else {
                    transaction?.rollback();
                }
            }
